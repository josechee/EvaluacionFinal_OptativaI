function  error = KNN_1(TRAIN, TEST, K)  ##TRAIN va ser la TRAIN original y TEST va ser TRAIN ya aplicado los algoritmos
  ...
  for i=1:length(TEST_clases)
    clasificar_esta_instancia = TEST(1,:);
    clase_actual = TEST_clases(i);
    #predecir_clase = clasificador(TRAIN,TRAIN_clases,clasificar_esta_instancia);
    %predecir_clase=KnnClasificador(TRAIN,TRAIN_clases,clasificar_esta_instancia,K);#knn multiples
    predecir_clase=KnnClasificador(TEST,TEST_clases,clasificar_esta_instancia,K);
